...
const wernerColors: WernerColor[] = [
  {
    "group": "Whites",
    "name": "Snow White",
    "hex": "#f1e9cd",
...
figma.ui.onmessage = async (msg: { type: string; color?: WernerColor }) => {
  if (msg.type === 'create-style' && msg.color) {
    // Create a paint style
    const style = figma.createPaintStyle();
    style.name = msg.color.name;
    style.paints = [{
      type: 'SOLID',
      color: hexToRgb(msg.color.hex)
    }];
  }
};
...
function showColorDetail(color) {
  ...
  const cleanDescription = color.description.replace(/\[(.*?)\]/g, '$1');
